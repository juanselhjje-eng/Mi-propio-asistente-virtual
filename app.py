...
DATA_DIR = Path(os.getenv("ASSISTANT_DATA_DIR") or ".jarvis_data")
...
class AssistantWorker(QObject):
    ...
    def run(self):
        try:
            ...
            for _ in range(8):
                ...
                if not calls:
                    answer = text or getattr(response, "output_text", "") or "Listo."
                    self.finished.emit(answer, ", ".join(a.name for a in team))
                    return
                agent.add_tool_outputs(calls)
            self.finished.emit("Detuve el proceso para evitar un ciclo demasiado largo.", ", ".join(a.name for a in team))
    ...
    def _instructions(self):
        ...
        return f"""Eres JARVIS, un constructor local de software. Idioma de respuesta: {language}. Personalidad: {personality}.
Construye proyectos reales dentro del workspace, no solamente ejemplos.
Para 'creame un juego/app/pagina de X', crea una estructura pequeña y funcional, conecta los archivos y valida el resultado.
Respeta el lenguaje pedido: Python, Pygame, HTML, CSS, JavaScript, TypeScript, C, C++, C#, Java, Rust, Go, PHP o SQL.
Si faltan sprites, imagenes, audio o iconos, crea assets y explica exactamente donde colocarlos. Usa placeholders generados por codigo cuando sea posible.
...
Usa validate_project en proyectos de varios archivos y valida Python cuando corresponda. No ejecutes programas interactivos indefinidamente.
No inventes APIs, dependencias, archivos, capturas ni resultados. Al terminar indica archivos creados/modificados, pruebas, errores corregidos y recursos pendientes."""
...
class MessageBubble(QFrame):
    def __init__(self, role, text, accent):
        ...
        title = QLabel("Tú" if role == "user" else "JARVIS")
        ...
        self.setStyleSheet(f"#userBubble {{ background:#EFF6FF; border:1px solid #DBEAFE; border-radius:16px; }} #assistantBubble {{ background:#FFFFFF; border:1px solid #E5E7EB; border-radius:16px; }} #messageTitle {{ color:{accent}; font-weight:700; }} #messageBody {{ color:#172033; font-size:14px; line-height:1.4; }}")
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings, parent=None):
        ...
        note = QLabel("Azul y blanco es el tema predeterminado. Los cambios se guardan localmente.")
        note.setWordWrap(True); note.setObjectName("muted")
        layout.addWidget(note)
        save = QPushButton("Guardar configuración"); save.clicked.connect(self.accept)
...
class JarvisWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("JARVIS · Local Developer Hub")
    ...
    def build_ui(self):
        ...
        brand = QLabel("JARVIS"); brand.setObjectName("brand")
        sub = QLabel("LOCAL DEVELOPER HUB"); sub.setObjectName("brandSub")
        side.addWidget(brand); side.addWidget(sub); side.addSpacing(14)
        new_btn = QPushButton("＋  Nueva conversación"); new_btn.clicked.connect(self.new_conversation); new_btn.setObjectName("newChat")
        ...
        self.chat_title = QLabel("Nueva conversación"); self.chat_title.setObjectName("chatTitle"); top_l.addWidget(self.chat_title)
        top_l.addStretch(); self.team_label = QLabel("Planner · Tester"); self.team_label.setObjectName("muted"); top_l.addWidget(self.team_label)
        center_layout.addWidget(top)

        self.stack = QStackedWidget(); self.chat_page = QWidget(); chat_layout = QVBoxLayout(self.chat_page); chat_layout.setContentsMargins(0, 0, 0, 0)
        self.scroll = QScrollArea(); self.scroll.setWidgetResizable(True); self.scroll.setFrameShape(QFrame.NoFrame)
        self.messages_widget = QWidget(); self.messages_layout = QVBoxLayout(self.messages_widget); self.messages_layout.setContentsMargins(40, 28, 40, 28); self.messages_layout.setSpacing(14); self.messages_layout.addStretch(); self.scroll.setWidget(self.messages_widget); chat_layout.addWidget(self.scroll, 1)

        composer = QFrame(); composer.setObjectName("composer"); cl = QVBoxLayout(composer); cl.setContentsMargins(24, 12, 24, 16)
        self.prompt = QTextEdit(); self.prompt.setPlaceholderText("Describe lo que quieres construir...\nEjemplo: créame un juego de naves en Python con Pygame"); self.prompt.setFixedHeight(92); self.prompt.installEventFilter(self); cl.addWidget(self.prompt)
        ...
        self.send = QPushButton("Crear  ↵"); self.send.setObjectName("send"); self.send.clicked.connect(self.send_prompt); row.addWidget(self.send); cl.addLayout(row); chat_layout.addWidget(composer)
        self.stack.addWidget(self.chat_page)
        center_layout.addWidget(self.stack, 1)
        main.addWidget(center, 1)

        inspector = QFrame(); inspector.setObjectName("inspector"); inspector.setFixedWidth(280)
        il = QVBoxLayout(inspector); il.setContentsMargins(16, 18, 16, 18)
        it = QLabel("Centro de proyecto"); it.setObjectName("panelTitle"); il.addWidget(it)
        self.project_status = QLabel("Listo para construir"); self.project_status.setObjectName("statusCard"); self.project_status.setWordWrap(True); il.addWidget(self.project_status)
        il.addSpacing(12)
        ...
        vision = QLabel("Mapeo de pantalla\nNo activo todavía. La interfaz queda preparada para conectar captura, análisis visual y control de pantalla sin fingir resultados."); vision.setObjectName("visionCard"); vision.setWordWrap(True); il.addWidget(vision)
        main.addWidget(inspector)
        self.setCentralWidget(root)

    def eventFilter(self, obj, event):
        if obj is self.prompt and event.type() == event.KeyPress and event.key() in (Qt.Key_Return, Qt.Key_Enter) and not (event.modifiers() & Qt.ShiftModifier):
    ...
    def apply_theme(self):
        ...
        QPushButton {{ border:1px solid #D7E0EC; border-radius:10px; padding:10px 14px; background:white; color:#172033; font-weight:600; }} QPushButton:hover {{ border-color:{accent}; background:#EFF6FF; }}
        #newChat {{ background:{accent}; color:white; border:none; }} #newChat:hover {{ background:#1D4ED8; }} #sideButton {{ text-align:left; }}
    ...
    def new_conversation(self):
        ...
        self.current_id = cid
        self.save_state(); self.refresh_history(); self.render_messages(); self.chat_title.setText("Nueva conversación"); self.prompt.clear(); self.project_status.setText("Listo para construir")

    def save_state(self): save_json(CONVERSATIONS_FILE, self.conversations); save_json(SETTINGS_FILE, self.settings)
    ...
    def render_messages(self):
        ...
        messages = self.conversations.get(self.current_id, {}).get("messages", [])
    ...
    def send_prompt(self):
        ...
        self.send.setEnabled(False); self.send.setText("Trabajando…"); self.project_status.setText("JARVIS está construyendo y comprobando el proyecto…")
        history = data["messages"][:-1]
        self.thread = QThread(); self.worker = AssistantWorker(history, text, self.settings.copy()); self.worker.moveToThread(self.thread); self.thread.started.connect(self.worker.run); self.worker.finished.connect(self.on_finished); self.worker.error.connect(self.on_error); self.thread.start()

    def on_finished(self, answer, team):
        data = self.conversations[self.current_id]; data["messages"].append({"role": "assistant", "content": answer}); data["updated"] = datetime.now().isoformat(); self.save_state(); self.team_label.setText(team.replace(", ", " · ")); self.activity.addItem("✓ Proyecto revisado"); self.project_status.setText("Proceso terminado. Revisa las pruebas y archivos indicados por JARVIS."); self.render_messages(); self.finish_worker()

    def on_error(self, error):
        self.activity.addItem("✕ Error: " + error); self.project_status.setText("Se produjo un error. Revisa que Ollama esté activo y que el modelo exista."); self.finish_worker()
        QMessageBox.warning(self, "JARVIS", error)
    ...
    def open_settings(self):
        ...
        dialog.changed.connect(self.update_settings); dialog.exec()
...
def main():
    ...
    app.setApplicationName("JARVIS Local Developer Hub")
    app.setFont(QFont("Segoe UI", 10))
    window = JarvisWindow(); window.show()
    return app.exec()
